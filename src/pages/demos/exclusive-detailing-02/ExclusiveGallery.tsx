      ...
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
        ...
        <FadeIn className="mb-16">
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant max-w-3xl border-l-2 border-primary pl-6">
            Real client photos can be added later. For now, this demo keeps the approved master website visuals in place to show how Exclusive Detailing 02’s results and gallery sections will appear.
          </p>
        </FadeIn>
            ...
            <FadeIn key={idx} delay={idx * 0.1}>
              <div className="bg-surface border border-outline-variant/10 rounded overflow-hidden shadow group relative border-b-4 border-b-transparent hover:border-b-primary transition-colors">
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-center items-center z-10 p-4 text-center">
                  <span className="text-white font-label-caps uppercase text-sm mb-2">Exclusive Detailing 02</span>
                  <span className="text-primary text-xs">Demo result visual</span>
                </div>
                <img src={src} alt="Exclusive Detailing 02 demo image — client photo to be added later" className="w-full h-64 object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
            </FadeIn>
          ))}
        </div>
